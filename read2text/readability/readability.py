...
from read2text.readability.BeautifulSoup import NavigableString
from read2text.readability.page_parser import parse, get_title, get_body, Unparseable


REGEXES = {
'unlikelyCandidatesRe': re.compile('combx|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor', re.I),
...
class Document:
    ...
    def select_best_candidate(self, candidates):
        ...
        self.debug(
            "Best candidate %s with score %s" % (describe(best_candidate['elem']), best_candidate['content_score']))
    ...
    def remove_unlikely_candidates(self):
        for elem in self.html.findAll():
            ...
            not REGEXES['okMaybeItsACandidateRe'].search(s)) and elem.name != 'body':
    ...
    def sanitize(self, node, candidates):
        ...
        for el in self.tags(node, "table", "ul", "div"):
            ...
            if weight + content_score < 0:
                el.extract()
                self.debug(
                    "Conditionally cleaned %s with weight %s and content score %s because score + content score was less than zero." %
                ...
                if counts["img"] > counts["p"]:
                    ...
                    to_remove = True
                elif content_length < (self.options.get('min_text_length', self.TEXT_LENGTH_THRESHOLD)) and (
